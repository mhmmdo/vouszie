			...
			const tools = [
				new DynamicTool({
					...
					func: async (image_id) => {
						try {
							const database = readDatabase();
							if (!database.images[image_id]) {
								return 'ID gambar tidak ditemukan';
							}

							const image = Buffer.from(database.images[image_id], 'base64');
				...
				new DynamicTool({
					...
					func: async (id) => {
						try {
							const database = readDatabase();
                			if (!database.images[id]) {
                    			return 'ID gambar tidak ditemukan';
                			}
                			
							const image = Buffer.from(database.images[id], 'base64');
